...
「子午流注 · 十二时辰经络图」：中医时辰经络主题的纯前端单页应用。实时展示当前时辰的经络当令、养生宜忌与《黄帝内经》金句，右侧 24 小时制地支表盘时钟指针实时流注，底部时间轴可拖动预览全天变化。
...
- 例外：表盘 SVG 内部（扇区/刻度/指针/盘心圆牌着色）保留 `index.css` 中的场景样式类（`.clock-*`），因其依赖运行时 CSS 变量与 SVG 特性
- 响应式尺寸用任意值 `clamp()`（如 `text-[clamp(22px,4vh,38px)]`）；桌面正常视窗下**整页单屏无滚动**（根容器 `h-dvh` + `overflow-hidden`），请勿改回 `min-h-*` 布局；窄屏用 `max-[900px]:` 变体恢复堆叠与滚动；宽度 ≤640px 时表盘时钟隐藏（`max-[640px]:hidden`），仅保留时辰面板与时间轴
- 表盘指针不用 CSS transition/animation：角度由每秒 tick 直接计算 `rotate` 变换，避免跨 0°（子时起点）时指针倒转回卷
...
- 可访问性底线：交互元素（时间轴段、表盘扇区、按钮）必须键盘可达并有可见 `:focus-visible` 反馈；SVG 内元素（如表盘扇区）禁用全局 outline（会在 `<g>` 包围盒画出方框），改用提亮等状态反馈；金句等动效须在 `prefers-reduced-motion: reduce` 下降级为静止
...
`src/data/meridians.ts` 的 `SHICHEN_LIST`（12 条）是全部内容的来源：经络、脏腑、五行、传统色（color/colorDeep/colorLight）、宜忌、金句（text/source/gloss）。新增或修改字段时，同步检查 `ShichenPanel`、`Timeline`、`ShichenClock` 的引用。
...
### 时辰表盘（ShichenClock.tsx）

- viewBox 固定 `0 0 400 400`；24 小时制模拟表盘：偏移分钟 `offset / 1440 * 360` 即时针角度，**0°（正上方）= 子时起点 23:00，顺时针**，与扩展图标轮盘几何一致
- 表盘只有时针与秒针，**不设分针**：分针小时标度与 24 小时外圈刻度不对应，分钟由盘心 24 小时制数字时钟（`offsetToClock`）读取；勿重新加回分针
- 几何全部由 `polar` / `sectorPath` 纯代码生成，无手工坐标；十二扇区与 `SHICHEN_LIST` 索引一一对应（扇区间留 1.2° 缝隙）
- 交互必须保留：扇区可 hover 提亮、显示名称（`<title>`）、点击/键盘 Enter・Space 预览对应时辰；当前扇区高亮 + 地支字反白 + 外缘强调弧；拖动时间轴时时针与盘心数字时钟跟随预览时间（预览模式下秒针隐藏）；注意 ≤640px 窄屏下该组件整体隐藏，勿在隐藏后仍依赖其交互
...
- 金句必须标注真实出处（书名 + 篇名），不杜撰引文；养生内容保持“科普级整理”表述
...
4. 时间轴：拖拽/点击切换时辰，面板与表盘指针同步，出现「回到现在」并可恢复
5. 点击表盘扇区可预览对应时辰，时针与盘心数字时钟同步切换
6. 主题三态切换正常，浅色/深色下表盘扇区与指针均清晰可见
7. 键盘 Tab 可依次聚焦主题按钮、表盘扇区、时间轴段，焦点反馈可见；Enter 可触发预览
