...
- 可访问性底线：交互元素（时间轴段、表盘扇区、按钮）必须键盘可达并有可见 `:focus-visible` 轮廓；金句等动效须在 `prefers-reduced-motion: reduce` 下降级为静止
...
- 交互必须保留：扇区可 hover 提亮、显示名称（`<title>`）、点击/键盘 Enter・Space 预览对应时辰；当前扇区高亮 + 地支字反白 + 外缘强调弧；拖动时间轴时指针跟随预览时间（预览模式下秒针隐藏）；注意 ≤640px 窄屏下该组件整体隐藏，勿在隐藏后仍依赖其交互
...
5. 点击表盘扇区可预览对应时辰，指针与盘心地支字/经络名同步切换
