...
- **时辰表盘时钟**：24 小时制模拟表盘，时针一天一圈，子时（23:00）居正上方顺时针排列十二地支彩色扇区（与扩展图标同款视觉）；当前时辰扇区高亮反白，盘心展示书法地支字与当令经络名，秒针实时走动
- **双向联动**：点击表盘任一扇区可预览对应时辰；底部时间轴（自子时 23:00 起 12 段）拖拽/点击时表盘指针同步转动，面板与表盘高亮随时辰切换，「回到现在」一键恢复实时
