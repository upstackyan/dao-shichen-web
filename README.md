...
一个以中医「子午流注」为主题的交互式 Web 页面：实时展示当前时辰对应的经络当令、养生宜忌与经典金句，右侧十二时辰表盘时钟（24 小时制地支盘）指针实时流注，底部时间轴可拖动预览全天十二时辰变化。
...
- **时辰表盘时钟**：24 小时制模拟表盘，时针一天一圈直指当令扇区，子时（23:00）居正上方顺时针排列十二地支彩色扇区（与扩展图标同款视觉）；当前时辰扇区高亮反白，盘心居中展示 24 小时制数字时钟，秒针实时走动；不设分针，避免小时标度与 24 小时外圈混淆
- **双向联动**：点击表盘任一扇区可预览对应时辰；底部时间轴（自子时 23:00 起 12 段）拖拽/点击时表盘时针与数字时钟同步切换，面板与表盘高亮随时辰切换，「回到现在」一键恢复实时
...
- Tailwind CSS v4（`@tailwindcss/vite` 插件）：布局/排版/主题全部使用工具类；昼夜主题变量通过 `@theme inline` 映射为 `bg-bg`、`text-ink`、`bg-accent` 等设计令牌；表盘 SVG 内部着色保留少量场景样式（`index.css`）
- 零 UI / 动画库依赖：表盘指针基于 SVG `rotate` 变换随每秒 tick 推进，时间轴拖拽基于原生 Pointer Events，尺寸随视窗用 `clamp()` 任意值弹性压缩
...
│   ├── ShichenClock.tsx       # 右侧：24h 地支表盘时钟（扇区点击预览 + 指针联动）
...
- 表盘几何为纯代码生成（扇区环形路径/时刻刻度/指针角度均在 `ShichenClock.tsx`，viewBox 400×400，子时起点居正上方顺时针）
